...
@Tag(
    name = "Payments",
    description = "Razorpay Test Mode checkout and server-side signature verification")
...
public class PaymentController {
  ...
  public PaymentController(
      PaymentRepository repository, RazorpayService razorpay, PaymentProcessor processor) {
  ...
  @Operation(
      summary = "Get Razorpay checkout details",
      description =
          "Returns the public Test Mode key and Razorpay order ID after inventory is reserved.")
            ...
            .orElseThrow(
                () ->
                    new ResponseStatusException(
                        HttpStatus.NOT_FOUND, "Payment checkout not found for this order"));
  ...
  @Operation(
      ...
      description = "Validates Razorpay's signature and emits the payment-completed Kafka event.")
  @ApiResponse(responseCode = "200", description = "Payment verified")
  @ApiResponse(
      responseCode = "400",
      description = "Payment signature or Razorpay order ID is invalid")
  public void verify(@PathVariable UUID orderId, @RequestBody Verify body) {
    processor.verify(
        orderId, body.razorpayOrderId(), body.razorpayPaymentId(), body.razorpaySignature());
  }

  public record Checkout(
      @Schema(description = "Razorpay Test Mode public key", example = "rzp_test_********")
          String keyId,
      @Schema(description = "Razorpay order ID used by Checkout", example = "order_********")
  ...
  public record Verify(
      ...
      @Schema(description = "HMAC signature returned by Razorpay Checkout")
