...
public class PaymentProcessor {
  ...
  PaymentProcessor(
      PaymentRepository repo, KafkaTemplate<String, Object> kafka, RazorpayService razorpay) {
    this.repo = repo;
    this.kafka = kafka;
  ...
  void pay(InventoryReservedEvent event) {
    ...
    Payment payment =
        repo.save(
            new Payment(event.orderId(), razorpay.create(event.orderId(), event.totalAmount())));
    log.info("payment.awaiting_razorpay_checkout orderId={}", event.orderId());
  }

  @Transactional
  public void verify(
      UUID orderId, String razorpayOrderId, String razorpayPaymentId, String signature) {
    Payment payment = repo.findById(orderId).orElseThrow();
    if (!payment.getRazorpayOrderId().equals(razorpayOrderId)
        || !razorpay.valid(razorpayOrderId, razorpayPaymentId, signature))
      throw new IllegalArgumentException("Invalid Razorpay signature");
    if (!"PAID".equals(payment.getStatus())) {
      payment.complete(razorpayPaymentId);
      kafka.send(
          "payments.completed.v1",
          orderId.toString(),
          new PaymentCompletedEvent(UUID.randomUUID(), orderId, razorpayPaymentId, Instant.now()));
    }
  }
}
