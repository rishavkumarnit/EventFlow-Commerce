...
public class OrderOutboxService {
    ...
    OrderOutboxService(OutboxEventRepository outboxEvents, ObjectMapper objectMapper) {
        this.outboxEvents = outboxEvents; this.objectMapper = objectMapper;
    }

    public void enqueue(OrderCreatedEvent event) {
        try {
            outboxEvents.save(new OutboxEvent(UUID.randomUUID(), event.orderId(), "orders.created.v1",
                    event.getClass().getName(), objectMapper.writeValueAsString(event), Instant.now()));
