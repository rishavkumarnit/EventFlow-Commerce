...
class InventoryReservedListener {
  ...
  @KafkaListener(topics = "payments.completed.v1", groupId = "order-service")
  @Transactional
  void markOrderPaid(PaymentCompletedEvent event) {
    orderRepository.findById(event.orderId()).ifPresent(order -> order.markPaid());
    log.info("order.paid orderId={} paymentId={}", event.orderId(), event.paymentId());
  }
}
