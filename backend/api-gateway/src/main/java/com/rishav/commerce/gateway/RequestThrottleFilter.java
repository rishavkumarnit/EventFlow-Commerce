 ...
 *
 * <p>This protects public API endpoints during local and single-instance deployments. For multiple
 * gateway instances, replace the in-memory map with a shared Redis-backed limiter so every instance
...
class RequestThrottleFilter implements GlobalFilter, Ordered {
  ...
  RequestThrottleFilter(
    ...
    this.rejectedRequests =
        Counter.builder("gateway_throttle_rejections_total")
  ...
  public Mono<Void> filter(ServerWebExchange exchange, GatewayFilterChain chain) {
    ...
    var result =
        clients
            .computeIfAbsent(clientKey(exchange), ignored -> new WindowCounter())
  ...
  private static final class WindowCounter {
    ...
    synchronized ThrottleResult tryAcquire(int limit) {
      ...
      var retryAfterSeconds =
          Math.max(1, (int) Math.ceil((60_000 - (now - windowStartedAt)) / 1_000.0));
      if (count >= limit) {
